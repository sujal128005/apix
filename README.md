...
`1047 tests passing · ruff clean · mypy --strict clean · CI on every push`
...
.venv\Scripts\pytest -q                           # expect 1047 passed
```
...
| `/heatmap` | Route-level movement across India, drawn without a tile server |
| `/scenario` | Fuel, demand and fare shocks translated to a CPI contribution range |
...
**1,047 tests** — 604 unit, 443 integration. The count is not the point; what it
covers is:
...
`docs/GIGW-READINESS.md` assesses conformity for Government of India hosting.
`docs/DATA-REQUEST.md` is what APIx needs from MoSPI and DGCA to run on live data.
`docs/DEPLOY-NOW.md` gets a live URL in about thirty minutes.
`docs/evidence/EVIDENCE-MANIFEST.md` regenerates every claim this project makes.
...
Every figure in this repository is either produced by a file in
`docs/evidence/` or regenerated by running something:

```powershell
.venv\Scripts\python scripts\build_evidence_pack.py
```

Twelve checks — golden index values, no bypass around the compliance gate, no
float on a money path, TLS verification, immutability, accessibility contrast,
reversible migrations, types, lint, outlier sensitivity, source permissions, and
the full suite. The manifest also records **four claims that cannot be verified
by running anything**, so that absence from the table never silently means *not
checked*.
...
.venv\Scripts\python scripts\check_source_permissions.py    # what each source permits
.venv\Scripts\python scripts\build_evidence_pack.py         # regenerate every claim
.venv\Scripts\python scripts\run_scheduler.py               # run collection continuously
```
